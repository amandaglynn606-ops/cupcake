...
Every public page now includes UAE-focused JSON-LD: Organization, WebSite, its page type and breadcrumbs where applicable. Service areas come from the same seven-emirate list used at checkout, and page language is `en-AE`. Collection ItemLists describe the cakes actually displayed on that page, including pagination. Priced cakes include Product and Offer data using the selected variation's published AED price, UAE-only eligibility and the existing regional delivery fees. Quote-only designs use Service data without an invented price. The FAQ schema uses the visible questions and answers. Private/noindex pages emit no promotional schema. No ratings, reviews, branch addresses, opening hours or delivery lead times are invented; Google decides rich-result eligibility and rankings.
