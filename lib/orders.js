...
function createOrder(input, catalog, now = new Date()) {
  ...
  if(input.cartDetails!==undefined){
    ...
    if(!d||typeof d!=='object'||!['show','anonymous'].includes(d.senderDisplay)||!['delivery','pickup'].includes(d.fulfilment)||!['10am-7pm','10am-10pm'].includes(d.time)||!['no','request'].includes(d.sameDay)||d.leadTimeAccepted!==true)throw new OrderError('Please complete the gift and delivery details in your cart.');
    ...
    cartDetails={giftMessage:clean(d.giftMessage,'gift message',200),senderDisplay:d.senderDisplay,instructions:clean(d.instructions??'','special instructions',1000,false),fulfilment:d.fulfilment,date,time:d.time,sameDay:d.sameDay,leadTimeAccepted:true};
  }
  return {
