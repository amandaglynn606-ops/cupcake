...
const reviews=read('extravagant-cake-review.json'),backgrounds=read('extravagant-background-review.json');
const candidates=read('luxury-cake-candidates.json');
const evidence={GB:'Source gallery identifies London, United Kingdom.',AU:'Source gallery identifies handmade cakes in Sydney, Australia.',IT:'Source page gives Via Livorno 236, Sesto San Giovanni (MI), Italy.',CA:'Source website identifies Toronto and the Greater Toronto Area, Ontario, Canada.'};
if(reviews.length!==10||new Set(reviews.map(r=>r.n)).size!==10)throw Error('Expected 10 unique reviewed designs');
(async()=>{
 ...
 try{
  ...
  for(const [index,r] of reviews.entries()){
   ...
   const handle=slug+'-e'+String(index+1).padStart(2,'0'),id=String(9920000000001+index);
   const image='assets/products/cakes/extravagant/cake-'+handle+'.webp';
   ...
   newBackgrounds.push({originalImage:c.localImage,products:[{id,title:r.title,page:'/cakes/'+handle}],status:'applied',batch:5,...common});
  }
 }finally{await browser.close();}
 if(new Set(records.map(r=>r.originalSha256)).size!==10||new Set(records.map(r=>r.sha256)).size!==10)throw Error('Duplicate photographs');
 write('extravagant-additions.json',{products});
 write('extravagant-addition-manifest.json',{createdAt:new Date().toISOString(),method:'Internet and Pinterest discovery followed by direct source downloads. Built-in imagegen background edits; generative fine details may vary. Visual review and exact-hash duplicate checks completed; dedicated reverse-image verification unavailable.',products:records});
 ...
 manifest.updatedAt=new Date().toISOString();manifest.scope='All 189 photos used by the 177 active cake designs. Cake tiers and decoration reviewed.';
 ...
 for(const r of reviews.filter(r=>r.flowerMaterial)){
  ...
  flowers.products[p.id]={material:r.flowerMaterial,flowerTypes:r.flowerTypes||['roses','mixed-blossoms'],verified:true,source,evidence:'Source identifies handmade sugar flowers; flower types visually reviewed.'};
 }
 write('flower-details.json',flowers);
 const selection=read('luxury-selection.json');
 selection.approvedProductIds=products.map(p=>p.id).concat(selection.approvedProductIds.filter(id=>!ids.has(id)));
 selection.updatedAt=new Date().toISOString();
 write('luxury-selection.json',selection);
