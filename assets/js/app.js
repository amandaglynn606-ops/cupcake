...
export async function renderCart(){
 ...
 if(!cart.length){containers.forEach(el=>el.innerHTML='<div class="cart-empty"><p class="eyebrow">YOUR NEXT OCCASION</p><h3>Your selection awaits.</h3><p>Explore the collection and find a design to make your own.</p><a class="button" href="/collections/all">Explore the collection →</a></div>');return;}
 try{
 ...
 }catch(error){if(request===cartRequest)containers.forEach(el=>el.innerHTML='<div class="cart-empty"><p class="form-error">'+esc(error.message)+'</p><button class="button button-outline" data-retry-cart>Try again</button><button class="text-link" data-clear-cart>Clear the saved bag</button></div>');}
}
