...
function syncFields(){
 ...
 for(const key of ['city','area','address']){form.elements[key].required=false;form.elements[key].disabled=pickup;}
...
async function refresh(){
 ...
 try{
  ...
  $('#checkout-empty').hidden=!!quote.items.length;$('#checkout-layout').hidden=!quote.items.length;
  ...
  $('#checkout-summary').innerHTML=`<div class="summary-heading"><div><p class="eyebrow">YOUR ORDER</p><h2>Order summary</h2></div><a href="/collections/all">Add cakes</a></div>${quote.hasStartingPrices?'<p class="starting-price-note">Starting design estimate. Final specifications and the complete quotation must be agreed on WhatsApp before ordering.</p>':''}${quote.items.map(item=>`<article class="summary-item"><a class="summary-photo" href="/cakes/${esc(item.handle)}"><img class="${item.cakeImage?'cake-image':''}" src="${esc(imageUrl(item.image))}" alt="${esc(item.imageAlt||item.title)}" width="80" height="96"><span aria-label="Quantity ${item.quantity}">${item.quantity}</span></a><div><h3>${esc(item.title)}</h3><p>${esc(item.variation)}</p>${personalisationText(item).map(text=>`<p class="summary-personalisation">${esc(text)}</p>`).join('')}${item.confirmationNotes?.map(note=>`<p class="summary-personalisation">${esc(note)}</p>`).join('')||''}${item.handle?`<a class="summary-edit" href="/cakes/${esc(item.handle)}?variant=${esc(item.variantId)}&amp;edit=${item.index}">Edit cake</a>`:''}<button type="button" class="cart-remove" data-remove="${esc(snapshot[item.index]?.lineId||'')}">Remove</button></div><span>${money(item.lineTotalFils)}</span></article>`).join('')}<div class="fee-row"><span>Subtotal · ${quote.items.reduce((n,i)=>n+i.quantity,0)} items</span><span>${money(quote.subtotalFils)}</span></div><div class="fee-row" data-delivery-fee><span>${quote.fulfilment==='pickup'?'Pickup':'Delivery · '+esc(quote.emirate)}</span><span>${money(quote.deliveryFeeFils)}</span></div><div class="summary-total" data-grand-total><span>Total</span><strong data-amount-only>${money(quote.totalFils)}</strong></div><p class="summary-note">Delivery across the UAE: AED 100 in Dubai; AED 200 in all other emirates. Pickup has no delivery fee.</p><div class="summary-whatsapp"><span class="eyebrow">CONFIRMED ON WHATSAPP</span><p>Your cake, date and delivery arrangements are confirmed personally with you.</p></div>`;
...
document.addEventListener('click',event=>{if(event.target.closest('#retry-checkout'))refresh();if(event.target.closest('[data-next-delivery]')){setDisclosure($('[data-checkout-contact]'),false);setDisclosure($('[data-checkout-delivery]'),true);$('[data-checkout-delivery] summary').focus({preventScroll:true});}});
form.addEventListener('submit',async event=>{
 event.preventDefault();if(pending||!getCart().length||!quote?.items.length)return;
 const invalid=[...form.elements].find(field=>field.validity&&!field.disabled&&!field.validity.valid);
 if(invalid){const section=invalid.closest('details');if(section)setDisclosure(section,true,{immediate:true});invalid.reportValidity();return;}
 ...
 finally{pending=false;button.disabled=false;button.textContent='Proceed to WhatsApp';}
});
