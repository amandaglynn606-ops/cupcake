...
async function refresh(){
 ...
 try{
  ...
  if(!quote.items.length){$('#checkout-layout').innerHTML='<div class="empty-state full-width"><h2>Your selection awaits.</h2><p>Choose a cake before preparing your WhatsApp order.</p><a class="button" href="/collections/all">Explore the collection &rarr;</a></div>';return;}
...
form.addEventListener('submit',async event=>{
 ...
 const payload=JSON.stringify({items:getCart(),customer,fulfilment:fields.fulfilment,preferredTime:fields.time,consent:form.elements.consent.checked,...(billingAddress?{billingAddress}:{}),...(cartDetails?{cartDetails:{...cartDetails,fulfilment:fields.fulfilment,emirate:fields.emirate,date:fields.date,time:fields.time,sameDay:fields.date===today?cartDetails.sameDay:'no'}}:{})});
