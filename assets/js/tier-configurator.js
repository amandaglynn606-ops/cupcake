...
import {setDisclosure,disclosureOpen,setPanelVisible} from './disclosure-motion.js';
...
export function showTier(n,focus=false,immediate=false){
 active=n;
 host.querySelectorAll('[data-tier-row]').forEach(row=>{const expanded=Number(row.dataset.tierRow)===n;setDisclosure(row,expanded,{immediate});if(expanded&&focus)row.querySelector('summary').focus({preventScroll:true});});
}
function refreshTiers(){
 ...
 if(copy)copy.disabled=first.querySelector('[data-tier-type]').value==='dummy'||![...first.querySelectorAll('.tier-flavours select')].every(s=>s.value);
}
export function renderTiers(product,variant){
 ...
 host.innerHTML='<div class="tier-builder">'+Array.from({length:count},(_,i)=>{
  const value=saved.get(i+1)||{type:'',sponge:'',filling:'',weightLb:''};
  return '<details class="tier-row" data-tier-row="'+(i+1)+'"><summary><span>Tier '+(i+1)+(i===0?' · Bottom':i===count-1?' · Top':'')+'<small data-tier-summary>Choose sponge & cream</small></span><span class="tier-toggle" aria-hidden="true">+</span></summary><div class="tier-content"><div class="tier-size-fields"><label><span>Tier type</span><select data-tier-type name="tier-'+(i+1)+'-type"><option value="">To confirm</option><option value="edible"'+(value.type==='edible'?' selected':'')+'>Edible cake</option><option value="dummy"'+(value.type==='dummy'?' selected':'')+'>Display tier · Not edible</option></select></label><label><span>Weight (lb) <small>Optional</small></span><input type="number" data-tier-weight name="tier-'+(i+1)+'-weight" min="0.5" max="500" step="0.5" inputmode="decimal" placeholder="e.g. 5" value="'+esc(value.weightLb??'')+'"'+(value.type==='dummy'?' disabled':'')+'></label></div><div class="tier-flavours"'+(value.type==='dummy'?' hidden':'')+'><label>Sponge<select data-tier-sponge name="tier-'+(i+1)+'-sponge"'+(value.type==='dummy'?' disabled':'')+'>'+options(choices.sponges,value.sponge)+'</select></label><label>Filling / cream<select data-tier-filling name="tier-'+(i+1)+'-filling"'+(value.type==='dummy'?' disabled':'')+'>'+options(choices.fillings,value.filling)+'</select></label></div><button type="button" class="tier-next" data-tier-next="'+(i+2)+'">'+(i<count-1?'Continue to tier '+(i+2)+' →':'Save tier choices')+'</button></div></details>';
 ...
 showTier(Math.min(active,count),false,true);refreshTiers();
}
host.addEventListener('click',event=>{
 const summary=event.target.closest('.tier-row > summary');
 if(summary){event.preventDefault();const row=summary.closest('[data-tier-row]'),expanded=!disclosureOpen(row);host.querySelectorAll('[data-tier-row]').forEach(other=>setDisclosure(other,other===row&&expanded));if(expanded)active=Number(row.dataset.tierRow);}
...
host.addEventListener('change',event=>{
 if(event.target.matches('[data-tier-type]')){
  ...
  setPanelVisible(row.querySelector('.tier-flavours'),!dummy);
  row.querySelector('[data-tier-weight]').disabled=dummy;
  row.querySelectorAll('.tier-flavours select').forEach(el=>{el.disabled=dummy;el.required=false;});
 }
 refreshTiers();
});
export function selectedTiers(){
 ...
 return [...host.querySelectorAll('[data-tier-row]')].map(row=>{
  ...
  const weight=type!=='dummy'?row.querySelector('[data-tier-weight]').value:'';
  return {tier:Number(row.dataset.tierRow),type,...(weight?{weightLb:Number(weight)}:{}),sponge:type!=='dummy'?row.querySelector('[data-tier-sponge]').value:'',filling:type!=='dummy'?row.querySelector('[data-tier-filling]').value:''};
 });
}
